...
Open with the normal bank button, an NPC using `openbank`, `@bank`, **Alt+B**, or **Ctrl+B** after logging into a character. **v2.4.1** restores the compact layout: Deposit, Withdraw, Diamond Buy/Sell and Ticket Buy/Sell are all visible together, with the original presets and Max menus. No action selector is needed. The panel measures 414 × 484 pixels including its border at 100%. See the [repeated-amount fix and verification](../../doc/bank_ui_reentry_20260921.md).

<p align="center">
  <img src="../../doc/images/bank-v2.4.1.png" width="412" height="482" alt="Bank v2.4.1 with all six actions visible and the original preset buttons">
</p>

*Native preview with synthetic balances.*
...
Item quantities start at **1**. Each item row shows its current Buy/Sell limits. Max opens the original menu for that row and only fills the amount. Comma-grouped whole numbers such as `1,000,000` are accepted. Returning to an amount removes grouping separators before editing; leaving the field restores them. Ctrl+A selects the entire amount. Appending, deleting or replacing digits after a transfer remains valid. Invalid grouping, decimals, negatives and overflow are rejected. Deposit wallet zeny first to fund purchases. Buy requires enough bank zeny and inventory space/weight; Sell requires eligible items and room in the bank. Character changes reset quantities and require fresh authenticated balances.
...
Build with `build.sh /absolute/output/path` using i686 MinGW, or `powershell -ExecutionPolicy Bypass -File build.ps1 -Output ABSOLUTE_DIRECTORY` using the installed MSYS2 mingw32 toolchain. The Windows build produces BankUI.dll, a native preview and five test programs. Run `bank_ui_test.exe` with `100`, `125` and `150` to check each supported scale. The loader test needs the installed FontScale loader and font DLL beside the candidate BankUI.dll and supports both current native reference fonts and the legacy scaler. `BankPreview.exe --scale=125` (or `150`) renders its own hidden window, including maximum balances, large totals, invalid quantities, insufficient funds and disconnection. All preview state is synthetic.

`bank_reentry_test.exe` uses native focus, edit, Backspace, Ctrl+A and button messages to repeat deposits and withdrawals and verify the exact submitted amounts. It reproduces the malformed-grouping failure against v2.4.
