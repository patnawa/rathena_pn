...
bool pc_jobchange(map_session_data *sd,int32 job, char upper)
{
	...
	// A same-lineage job change does not clear the skill tree. End Druid
	// transformations before replacing the class/body: val4 stores the old
	// body style and must not restore it after changing Karnos into Alitea.
	status_change_end(sd, SC_WEREWOLF);
	status_change_end(sd, SC_WERERAPTOR);
	...
	for (auto &jobIt : *this) {
		...
		do{
			...
			if( ( class_ & MAPID_SECONDMASK ) == MAPID_KAGEROUOBORO ||
				...
				( ( class_ & MAPID_SECONDMASK ) == MAPID_KARNOS && !pc_is_trait_job(class_) ) ){
